...
class Project(Base):
    ...
    # Agent configuration
    agent_endpoint: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    ...
    source_database: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    ...
    # GitHub configuration for code recommendations and issue creation
...
class Simulation(Base):
    ...
    # Analysis filters (for AgentSmith conversation analysis)
    ...
    conversation_ids: Mapped[Optional[List[str]]] = mapped_column(JSON, nullable=True)
    triggered_by: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    include_errors_only: Mapped[bool] = mapped_column(default=False)
...
class ImprovementSuggestion(Base):
    ...
    # Code recommendation generated by AI agent (optional)
    # Contains: file_changes, target_repo, target_path, github_issue_url, github_pr_url, status
    code_recommendation: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON, nullable=True
    )

    # Knowledge recommendation generated by AI agent (optional)
    # Contains: title, doc_type, target_doc, existing_doc_coverage, glean_sources, recommended_content, rationale, priority, status
    knowledge_recommendation: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON, nullable=True
    )
