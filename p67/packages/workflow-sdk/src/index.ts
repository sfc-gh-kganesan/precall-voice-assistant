...
export interface CortexCodeOptions {
    /** The prompt to send to Cortex Code (required) */
    prompt: string;
...
export interface WorkflowSDK {
    ...
    /**
     * Invokes the Cortex Code CLI to perform agentic coding tasks.
     *
     * Runs the `cortex` CLI as a subprocess with the given prompt and returns
     * the output. Cortex Code is an AI coding agent that can read/write files,
     * run commands, search code, and interact with Snowflake.
     *
     * @param options - Cortex Code invocation options:
     *   - `prompt`: The prompt/instruction to send (required)
     *   - `timeout`: Timeout in seconds (default: 900 = 15 min)
     *   - `workDir`: Working directory for the process (default: system temp)
     *   - `model`: Model to use (e.g., 'opus', 'sonnet')
     *   - `allowAllToolCalls`: Skip tool-call confirmations (default: false)
     * @returns Response with success status, output text, and optional error.
     *          Never throws - errors are returned in the response object.
     *
     * @example
     ...
     *
     * @example
     * // Use a specific model with full tool access
     * const response = await sdk.cortexCode({
     ...
     * });
     */
    cortexCode(options: CortexCodeOptions): Promise<CortexCodeResponse>;
}
