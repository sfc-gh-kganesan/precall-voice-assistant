...
DATABASE_URL="${DATABASE_URL:-postgresql://postgres:password@localhost:5432/controld_dev?schema=public}"

echo "🌱 Seeding test data for p67 dash..."
echo ""
...
echo ""
echo "✅ Seed complete! Test data has been created."
echo ""
echo "Summary:"
echo "  - 1 test user (TEST_USER)"
echo "  - 4 workflows (data-pipeline, notification-sender, report-generator, slack-approval)"
echo "  - 5 workflow runs (Completed, Running, Failed, Completed, Interrupted)"
echo "  - Multiple logs for each run"
echo "  - 2 interrupts (1 Pending, 1 Resumed)"
echo "  - 1 test secret"
echo ""
echo "Visit http://localhost:3001 to see the dash UI with test data."
