...
# Build find pipeline with optional limit
if [ -n "$LIMIT" ]; then
  file_stream=$(find "$TEMP_DIR" -type f -name "*.pdf" | head -n $LIMIT)
else
  file_stream=$(find "$TEMP_DIR" -type f -name "*.pdf")
fi

# Process each file
echo "$file_stream" | while read -r pdf_file; do
