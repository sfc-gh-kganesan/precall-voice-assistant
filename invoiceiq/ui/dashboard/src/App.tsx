...
export default function App() {
  ...
  const [optimisticUpdate, setOptimisticUpdate] = useState<
    { invoiceIds: string[]; newStatus: string; timestamp: number } | undefined
  >();
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (mappedInvoices.length === 0) {
        ...
        toast.success(
          `Found ${mappedInvoices.length} invoice${mappedInvoices.length !== 1 ? 's' : ''}`
        );
      }
    } catch (error) {
  ...
  const handleBulkStatusUpdate = async (invoiceIds: string[], newStatus: Invoice['status']) => {
    ...
    try {
      ...
      toast.success(
        `${response.updated_count} invoice${response.updated_count !== 1 ? 's' : ''} moved to ${newStatus}`
      );
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            <div>
              <h1 className="mb-1 bg-gradient-to-r from-[var(--snowflake-blue)] to-[var(--snowflake-teal)] bg-clip-text text-transparent">
                InvoiceIQ Dashboard
              </h1>
