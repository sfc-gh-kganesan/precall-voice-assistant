...
class OverallState(BaseModel):
    ...
    call_activity_previously_processed: bool = Field(default=False, description="True if potential new use cases discussed in the call transcript have already been inserted into the new uses cases table, False otherwise.")
    ...
    new_use_case_dedup_results: list[DedupeStructuredOutputState] = Field(default_factory=list, description="List of deduplication results for the new use cases, including the names of the proposed new use case and the names of the existing use cases that are a probable duplicate of the proposed new use cases.")
...
# TODO: Maybe create a node called find_most_similar_sfdc_use_case that uses cortex search to find the most similar SFDC use case (or 2) to the new use case and saves the ID in a col.
...
def new_use_case_insert(state: OverallState) -> OverallState:
    ...
    if len(new_use_cases_list) > 0 and not call_already_exists_in_new_use_cases_table:
        for new_use_case in new_use_cases_list:
            ...
            # ------------------------------------------------------------
            # # TODO: Uncomment this section when sfdc table is back:
            ...
            # # TODO: Delete this section when sfdc table is back.
            previous_use_cases_df = session.sql(f"""
            with extracted_use_case_summaries as (
                select RECORD_CREATION_DTTM,
                    new_use_case_id,
