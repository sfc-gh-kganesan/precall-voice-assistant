...
def render_unified_nav(
    ...
    """Render unified navigation bar for query and chunk navigation."""
    if total_queries == 0:
        return 0, 0

    query_idx = min(query_idx, total_queries - 1)
    chunk_idx = min(chunk_idx, total_chunks - 1) if total_chunks > 0 else 0

    col_query, col_spacer, col_prev, col_status, col_next = st.columns([3, 1, 0.5, 1.5, 0.5])

    with col_query:
        jump_key = f"{session_key}_query_jump"
        ...
        if selected_query != query_idx:
            dispatch(SetCarouselIndexAction(index=selected_query))
            dispatch(SetFeedbackChunkIndexAction(index=0))
            st.rerun()

    with col_prev:
        if st.button("◀", disabled=chunk_idx == 0, key=f"{session_key}_chunk_prev", use_container_width=True):
            dispatch(SetFeedbackChunkIndexAction(index=chunk_idx - 1))
            st.rerun()

    with col_status:
        st.markdown(f"<div style='text-align:center; padding-top:6px;'><b>Chunk {chunk_idx + 1} / {total_chunks}</b></div>", unsafe_allow_html=True)

    with col_next:
        if st.button("▶", disabled=chunk_idx >= total_chunks - 1, key=f"{session_key}_chunk_next", use_container_width=True):
            dispatch(SetFeedbackChunkIndexAction(index=chunk_idx + 1))
            st.rerun()

    return query_idx, chunk_idx
...
def render_articles_carousel(
    ...
    if total_articles > 1:
        ...
        with col_prev:
            if st.button("◀ Prev", disabled=current_idx == 0, key=f"{prefix}_prev", use_container_width=True):
                dispatch(index_action_class(index=current_idx - 1))
                st.rerun()
        with col_status:
        ...
        with col_next:
            if st.button("Next ▶", disabled=current_idx >= total_articles - 1, key=f"{prefix}_next", use_container_width=True):
                dispatch(index_action_class(index=current_idx + 1))
                st.rerun()
...
def render_feedback_carousel(session: Session, source_df: pd.DataFrame, feedback_counts_df: pd.DataFrame | None = None) -> None:
    ...
    with tab_chunks:
        ...
        if total_chunks > 1:
            col_prev, col_status, col_next = st.columns([1, 2, 1])
            with col_prev:
                if st.button("◀ Prev", disabled=current_chunk_idx == 0, key="chunk_prev", use_container_width=True):
            ...
            with col_next:
                if st.button("Next ▶", disabled=current_chunk_idx >= total_chunks - 1, key="chunk_next", use_container_width=True):
                    dispatch(SetFeedbackChunkIndexAction(index=current_chunk_idx + 1))
                    st.rerun()
